...
glass.onclick = () => {
  ...
  width = 10;
  const setAnimation = setInterval(increaseWidth, 50);
...
btn1.addEventListener("click", (e) => {
  try {
    ...
    if (
      btn2.classList.contains("active-bg") ||
      btn3.classList.contains("active-bg")
    ) {
      btn2.classList.remove("active-bg");
      btn3.classList.remove("active-bg");
      btn1.classList.add("active-bg");
    }
    const searchstring = Portfolio.filter((item) => {
      return (
        item.categories.includes(targets) || item.categories.includes(targets1)
      );
    });
...
btn2.addEventListener("click", (e) => {
  try {
    ...
    if (
      btn1.classList.contains("active-bg") ||
      btn3.classList.contains("active-bg")
    ) {
      btn1.classList.remove("active-bg");
      btn3.classList.remove("active-bg");
      btn2.classList.add("active-bg");
    }
    const searchstring = Portfolio.filter((item) => {
